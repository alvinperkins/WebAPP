...
import web.debugerror as debugerror

from web.utils import lstrips, safeunicode
import sys
...
class application:
    ...
    def __init__(self, mapping=(), fvars={}, autoreload=None):
        if autoreload is None:
            autoreload = webapi.config.get('debug', False)
    ...
    def _load(self):
        webapi.ctx.app_stack.append(self)
        
    def _unload(self):
        webapi.ctx.app_stack = webapi.ctx.app_stack[:-1]
        
        if webapi.ctx.app_stack:
            # this is a sub-application, revert ctx to earlier state.
            oldctx = webapi.ctx.get('_oldctx')
            if oldctx:
                webapi.ctx.home = oldctx.home
                webapi.ctx.homepath = oldctx.homepath
                webapi.ctx.path = oldctx.path
                webapi.ctx.fullpath = oldctx.fullpath
    ...
    def request(self, localpart='/', method='GET', data=None,
        ...
        To use https, use https=True.
            ...
            >>> app = application(urls, globals())
            >>> class redirect:
            ...     def GET(self): raise webapi.seeother("/foo")
            ...
        ...
        such as a dict.
            ...
            >>> class uaprinter:
            ...     def GET(self):
            ...         return 'your user-agent is ' + webapi.ctx.env['HTTP_USER_AGENT']
            ... 
            >>> app = application(urls, globals())
    ...
    def handle(self):
        fn, args = self._match(self.mapping, webapi.ctx.path)
    ...
    def wsgifunc(self, *middleware):
        ...
        def wsgi(env, start_resp):
            ...
            try:
                ...
                if webapi.ctx.method.upper() != webapi.ctx.method:
            ...
            result = webapi.safestr(iter(result))

            status, headers = webapi.ctx.status, webapi.ctx.headers
    ...
    def _delegate_sub_application(self, dir, app):
        ...
        """
        webapi.ctx._oldctx = webapi.storage(webapi.ctx)
        webapi.ctx.home += dir
        webapi.ctx.homepath += dir
        webapi.ctx.path = webapi.ctx.path[len(dir):]
        webapi.ctx.fullpath = webapi.ctx.fullpath[len(dir):]
        return app.handle_with_processors()
            
    def get_parent_app(self):
        if self in webapi.ctx.app_stack:
            index = webapi.ctx.app_stack.index(self)
            if index > 0:
                return webapi.ctx.app_stack[index-1]
    ...
    def internalerror(self):
        ...
        elif webapi.config.get('debug'):
            return debugerror.debugerror()
        else:
            debugerror.breakpad()
            return webapi._InternalError()
...
class subdomain_application(application):
    ...
    """
    def handle(self):
        host = webapi.ctx.host.split(':')[0] #strip port
