...
from web.utils import storage, safeunicode, safestr, re_compile
from web.webapi import config
    ...
    def read_expr(self, text, escape=True):
        ...
        def attr_access():
            ...
            if tokens.lookahead2().type == NAME:
                tokens.__next__() # consume dot
                identifier()
                extended_expr()
        
        def paren_expr():
            begin = tokens.__next__().value
            end = parens[begin]
            while True:
    ...
    def compile_template(self, template_string, filename):
        ...
        if not sys.platform.startswith('java'):
            try:
                ...
                #SafeVisitor().walk(astree, filename)
            except ImportError:
